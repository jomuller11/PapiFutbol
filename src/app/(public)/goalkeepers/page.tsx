...
export const metadata = {
  ...
  description: 'Arqueros con menos goles en contra en el torneo.',
};
...
export default async function GoalkeepersPage() {
  ...
  const { data: rows } = await supabase
    .from('stats_goalkeepers')
    .select('rank, player_name, team_name, goals_against')
    .order('rank', { ascending: true });

  const goalkeepers = (rows as any[]) ?? [];
  ...
  const top3 = goalkeepers.slice(0, 3);
              ...
              return (
                <div key={gk.rank} className="flex flex-col items-center">
                  ...
                  <div className={`w-full ${heightClass} flex flex-col items-center justify-center ${podiumColor}`}>
                    ...
                    <div className="font-mono text-[10px]">{gk.goals_against} GC</div>
                  </div>
                </div>
              );
            })}
        ...
        {goalkeepers.map((gk, i) => (
          ...
          >
            ...
            <div className="text-right flex-shrink-0">
              <div className="font-display text-xl text-emerald-700 leading-none">{gk.goals_against}</div>
              <div className="font-mono text-[9px] text-slate-400 mt-1 tracking-wider">GC</div>
            </div>
          </div>
        ))}
      </div>
...
function EmptyState() {
  return (
    <div className="text-center py-16 text-slate-500 px-6">
      ...
      <h1 className="font-serif text-xl font-bold text-slate-900 mb-2">Valla menos vencida</h1>
      <p className="text-sm font-medium">Los datos aparecerán cuando se sincronicen las estadísticas.</p>
    </div>
  );
}
